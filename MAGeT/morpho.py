...
class Morphobits:
	...
	def build_pipeline(self,atlases,native_subjects,native_templates,model,tasklist=None):
		...
		for s in subjects:
			subject_tasklist = self.per_subject_pipeline(atlases,s,templates,model)
			tasklist.command('per_subject', subject_tasklist)
		return tasklist

	def per_subject_pipeline(self,atlases,subject,templates,model):
		tasklist = taskset()
		...
		for a,t in product(atlases,templates):
			...
			assert xfms is not [], "{model}, {a}, {t}, {subjec} are all the same image?".format(**vars())
		...
		for o in model.objects():
			obj_displace='{displacement_dir}/{subject.stem}_{o.stem}.txt'.format(**vars())
